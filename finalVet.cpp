...
ARVORE *InsereNO(ARVORE *Raiz, ARVORE *R, int mat, int end);
...
void main(void)
{
   ...
   else
   {
		...
		else
		{
         ...
         Res = InsereNO(Raiz, Raiz, mat, aend);
			if (!Res) strcat(msg, "\n\tERRO: O no nao foi alocado com sucesso!");
			Raiz = Res;
         while(!feof(Ac))
   ...
   {
      fread(&Reg, sizeof(Reg), 1, Ac);
      if (Reg[0].Matricula != 0) strcat(msg, "\n\tArquivo de dados carregado com sucesso de AGENDA.DAT.");
   ...
   do
   {
   	...
   	switch(op)
      {
      	case 1:  // Insere um registro
                  ...
                  strcpy(Reg[end].Telefone, fone);
                  end++;

                  // Inserção no arquivo de chaves
                  Res = InsereNO(Raiz, Raiz, mat, end);
						if (!Res)
                  {
         ...
         case 0:  // Sair
                  ...
                  if (Reg[0].Matricula != 0) fwrite(&Reg, sizeof(Reg), 1, Ac);
					   fclose(Ac);
