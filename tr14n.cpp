...
void main(void) {
  ...
  printf("\n\n     O programa funciona com: (, ), [, ], {, }, +, -, *, / e numeros inteiros");
  printf("\n\n\tDigite a expressao: ");
  gets(Exp);
  ...
  for (int i=0; (i<tam)&&(ret!=-2); i++) {
    switch(Exp[i]) {
      ...
      default:
        ...
        for (j=1, vfor=0; vfor!=-1;) {
          if ((Exp[i+1]!='{')&&(Exp[i+1]!='[')&&(Exp[i+1]!='(')&&(Exp[i+1]!='}')&&(Exp[i+1]!=']')&&(Exp[i+1]!=')')&&(Exp[i+1]!='+')&&(Exp[i+1]!='-')&&(Exp[i+1]!='*')&&(Exp[i+1]!='/')) {
            cnum[j] = Exp[i+1];
            i++;
            j++;
          }
